...
func wireApp(confServer *conf.Server, confData *conf.Data, auth *conf.Auth, logger *slog.Logger) (*kratos.App, func(), error) {
	...
	adminService := service.NewAdminService(adminUsecase, roleUsecase, adminRoleUsecase, permissionUsecase, groupPermissionUsecase)
	grpcServer := server.NewGRPCServer(confServer, todoService, loginService, adminService)
	httpServer := server.NewHTTPServer(confServer, todoService, loginService, adminService)
