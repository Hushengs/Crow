...
SELECT id, group_id, permission_id, create_date, update_date
...
func (r *groupPermissionRepo) DeleteGroupPermission(ctx context.Context, id int64) error {
	result, err := r.data.db.ExecContext(ctx, "DELETE FROM group_permission WHERE id = ?", id)
	if err != nil {
		return err
	}
	rowsAffected, err := result.RowsAffected()
